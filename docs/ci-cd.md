...
4. **Set image reference** — lowercases the owner and composes
   `ghcr.io/<owner>/kali-base:latest` into a step output.
...
```bash
just build   # produces kali-base:latest
just chunk   # produces kali-base:latest-chunked and prints layer counts
```
...
- Uses `dataaxiom/ghcr-cleanup-action` (pinned by commit SHA) to delete
  images older than 30 days from the `kali-base` package, keeping the last
